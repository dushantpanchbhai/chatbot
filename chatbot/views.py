    ...
    while(flag==True):
        if(user_response!='bye'):
            if(user_response=='thanks' or user_response=='thank you' ):
                flag=False
                return ("Bot: You're welcome!, Chat with you later !")
            else:
                if(greeting(user_response)!=None):
                    return ("Bot: "+greeting(user_response)+" how may i help you?")
                else:
                    try:
                        return ("Bot: "+chatbot_query(user_response,0))
                    except:
                        return ("bot2: "+chatbot_query(user_response,1))
        else:
            flag=False
...
def answer(request):
    ...
    word=r.split()
    ans=chat(word)
    return render(request,'ans.html',{'msg':ans})
